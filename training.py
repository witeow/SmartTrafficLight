...
#################################################################################################
#Gpyopt attempt
# space = [{'name': 'n_steps', 'type': 'discrete', 'domain': (32, 64, 128, 256)},
#          {'name': 'gamma', 'type': 'continuous', 'domain': (0.9, 0.99)},
#          {'name': 'learning_rate', 'type': 'continuous', 'domain': (1e-5, 1e-3)},
#          {'name': 'ent_coef', 'type': 'continuous', 'domain': (0.0, 0.1)},
#          {'name': 'vf_coef', 'type': 'continuous', 'domain': (0.0, 1.0)},
#          {'name': 'max_grad_norm', 'type': 'continuous', 'domain': (0.1, 1.0)}]
...
#     shutil.copy(srcFlow, desFlow)
#     shutil.copy(srcRoadnet, desRoadnet)
#     cityflowEnv = cityFlowGym("sample_data/jacob_config.json", 3600, logOutFolder, 100)
#     log_path = os.path.join('Optimizing', 'Logs')
...
#                 ent_coef=params[0][3],
#                 vf_coef=params[0][4],
#                 max_grad_norm=params[0][5])
    
#     # Train the PPO agent
#     model.learn(total_timesteps=36000)
#     models_dir = "models/PPO_360000_" + timestamp + "_optimizing"
#     model.save(f"{models_dir}")
#     # Evaluate the PPO agent
#     mean_reward = 0
#     for i in range(10):
...
# opt = GPyOpt.methods.BayesianOptimization(f=ppo_objective, domain=space)

# # Run the optimization
# opt.run_optimization(max_iter=10)

# # Find the index of the best set of hyperparameters
# best_index = opt.Y.argmax()

# # Extract the best hyperparameters
# best_params = opt.X[best_index]

# # Store the best hyperparameters in a dictionary
# best_hyperparams = {
#     'n_steps': int(best_params[0]),
#     'gamma': best_params[1],
#     'learning_rate': best_params[2],
#     'ent_coef': best_params[3],
#     'vf_coef': best_params[4],
#     'max_grad_norm': best_params[5]
# }
...
#     os.makedirs(logOutFolder)
# #open cityflow config file into dict
# configDict = json.load(open("sample_data/sample_config.json"))
# configDict["dir"] = logOutFolder + '/'
# with open("sample_data/sample_config.json", "w") as outfile:
#     json.dump(configDict, outfile)

# srcFlow = "sample_data/sample_flow.json"
# srcRoadnet = "sample_data/sample_roadnet.json"
# desFlow = logOutFolder + "/sample_flow.json"
# desRoadnet = logOutFolder + "/sample_roadnet.json"
# shutil.copy(srcFlow, desFlow)
# shutil.copy(srcRoadnet, desRoadnet)
# cityflowEnv = cityFlowGym("sample_data/sample_config.json", 3600, logOutFolder, 100)
# log_path = os.path.join('Training', 'Logs')

# # check_env(cityflowEnv, warn=True, skip_render_check=True)

# model = PPO("MultiInputPolicy", cityflowEnv, verbose=1, tensorboard_log=log_path, ent_coef=0.01)

# model.learn(total_timesteps=360000)
# models_dir = "models/PPO_360000_" + timestamp + "_sampleConfig"
# model.save(f"{models_dir}")